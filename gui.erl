...
-export([start_link/0, add_monkey/4, delete_monkey/1, add_balloon/3, delete_balloon/1, move_balloon/2,
         add_dart/3, delete_dart/1, move_dart/2, change_bananas/1,
         refresh/0]).
...
init([]) ->
    ...
    BananaImg = wxImage:new(ImagePath ++ "/banana.png"), BananaIcon = wxBitmap:new(wxImage:scale(BananaImg, 32, 32)), wxImage:destroy(BananaImg),
    ...
    wxWindow:connect(GroundMonkeyButton, command_button_clicked), wxWindow:connect(WaterMonkeyButton, command_button_clicked), wxWindow:connect(FireMonkeyButton, command_button_clicked),
    wxWindow:connect(AirMonkeyButton, command_button_clicked), wxWindow:connect(AvatarMonkeyButton, command_button_clicked), wxWindow:connect(StartWaveButton, command_button_clicked),
    wxFrame:show(Frame),
...
handle_cast({change_bananas, A}, S=#state{banana_text_widget=W}) -> wxStaticText:setLabel(W,": "++integer_to_list(A)), {noreply, S};
...
handle_info(#wx{event=#wxMouse{type=left_down, x=_X, y=_Y}}, S=#state{placing=none}) -> io:format("GUI: Please select a monkey type first.~n"), {noreply,S};
