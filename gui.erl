...
handle_cast(lose_game,S=#state{frame=F})->Dialog=wxMessageDialog:new(F,"You lose! Play again?",[{caption,"Game Over"},{style,?wxYES_NO}]),Result=wxMessageDialog:showModal(Dialog),wxMessageDialog:destroy(Dialog),if Result==?wxID_YES->main_server:restart_game();Result==?wxID_NO->wxFrame:close(F)end,{noreply,S};
handle_cast(clear_board,S)->{noreply,S#state{monkeys=#{},balloons=#{},darts=#{}}};
