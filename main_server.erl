...
-export([start_link/0, stop/0, generate_level1/0, add_bananas/1,
         lose_game/0, restart_game/0]). % NEW
...
lose_game() -> gen_server:cast(?MODULE, lose_game).
restart_game() -> gen_server:cast(?MODULE, restart_game).

%% API Layer for GUI Communication
...
reset_ets() -> ets:delete_all_objects(game_objects), ets:insert(game_objects, {meta, bananas, ?STARTING_BANANAS}).

%% Server Callbacks
init([])->
    ets:new(game_objects,[set,public,named_table,{keypos,#balloon.id}]),
    rand:seed(exsplus),
    ets:insert(game_objects,{meta,bananas,?STARTING_BANANAS}),
    {ok,_} = gui:start_link(),
    change_bananas(?STARTING_BANANAS),
...
handle_cast(lose_game, State = #{game_over := false}) -> io:format("SERVER: Game Over!~n"), gui:lose_game(), {noreply, State#{game_over => true}};
handle_cast(lose_game, State) -> {noreply, State};
handle_cast(restart_game, _State) -> io:format("SERVER: Restarting game...~n"), reset_ets(), gui:clear_board(), change_bananas(?STARTING_BANANAS), gui:refresh(), {noreply, #{game_over => false}};
handle_cast(stop,S)->{stop,normal,S};
handle_cast(_,S)->{noreply,S}.

handle_info(tick, State = #{game_over := false}) ->
    ...
    lists:foldl(fun(M,D)->handle_one_monkey(M,FreshBalloons,D)end,CurrentDarts,AllMonkeys),
    gui:refresh(),
    erlang:send_after(?TICK_RATE,self(),tick),
    {noreply,State};
handle_info(tick, State = #{game_over := true}) ->
    erlang:send_after(?TICK_RATE, self(), tick),
    {noreply, State};
handle_info(_,S)->{noreply,S}.

move_one_balloon(B=#balloon{id=I,path=P,path_index=Idx})->NI=Idx+1,if NI>length(P)->ets:delete(game_objects,I),delete_balloon(I),lose_game();true->ets:insert(game_objects,B#balloon{path_index=NI}),{NX,NY}=lists:nth(NI,P),move_balloon(I,{NX,NY})end.
...
handle_one_monkey(M=#monkey{cooldown=C,x=Mx,y=My,range=R},Balloons,Darts)->NC=if C>0->C-1;true->0 end,ets:insert(game_objects,M#monkey{cooldown=NC}),if NC==0,length(Balloons)>0->BInRange=lists:filter(fun(_B=#balloon{path=P,path_index=I})->BPos=lists:nth(I,P),calculate_distance({Mx,My},BPos)=<R end,Balloons),TgtIds=[Dart#dart.target_id||Dart<-Darts],AvailTgts=lists:filter(fun(_B=#balloon{id=Id})->not lists:member(Id,TgtIds)end,BInRange),case AvailTgts of[FirstTgt|_]->ets:insert(game_objects,M#monkey{cooldown=?MONKEY_COOLDOWN}),NewDart=shoot_dart(M,FirstTgt),[NewDart|Darts];[]->Darts end;true->Darts end.
shoot_dart(#monkey{x=Sx,y=Sy,type=MT}, #balloon{id=TID,path=P,path_index=Idx})->DI=make_ref(),DT=get_dart_type(MT),TPos=lists:nth(Idx,P),DP=calculate_path({Sx,Sy},TPos,?DART_STEPS),ND=#dart{id=DI,type=DT,path=DP,target_id=TID},ets:insert(game_objects,ND),gui:add_dart(DI,DT,{Sx,Sy}),ND.
terminate(_,_) -> ok.
